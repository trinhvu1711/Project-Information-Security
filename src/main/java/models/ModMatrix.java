...
public class ModMatrix {
    ...
    private final BigInteger mod= new BigInteger(String.valueOf(VietnameseTextHelper.ALPHABET_SIZE));
    ...
    public int[][] getInverseModulo(){
        int[][] result = new int[getNrows()][getNrows()];
        for (int i = 0; i < getNrows(); i++) {
            for (int j = 0; j < getNcols(); j++) {
                result[i][j] = getData()[i][j].intValue();
            }
        }
        return result;
    }
    public static void main(String[] args) {
        ...
        key2[0][0] = new BigInteger("4");
        key2[0][1] = new BigInteger("7");
        key2[1][0] = new BigInteger("2");
